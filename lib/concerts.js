...
function isHidden(concert) {
  return concert.ticketswapUrl &&
    [...HIDDEN_SLUGS].some(slug => concert.ticketswapUrl.includes(`/${slug}/`));
}
...
async function getConcerts(dateStr) {
  return (load().concerts || []).filter(c => c.date === dateStr && !isHidden(c));
}
